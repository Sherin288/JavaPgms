...
public class ExcelCode { 
 	...
 	static XSSFWorkbook w; // to represent workbook
 	static XSSFSheet sh; // to represent sheet in a workbook
  
 	public static String readStringData(int row, int col) throws IOException  { 
 		f = new FileInputStream("E:\\Obscura\\Testdata\\TestData1.xlsx"); 
 	...
 	public static String readIntegerData(int row, int col) throws IOException { 
 		f = new FileInputStream("E:\\Obscura\\Testdata\\TestData1.xlsx"); 
 		w = new XSSFWorkbook(f); // to load excel file to apache file to memory
 		sh = w.getSheet("Sheet1"); 
 		XSSFRow r = sh.getRow(row); 
 		XSSFCell c = r.getCell(col); 
